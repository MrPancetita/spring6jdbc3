...
//@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT) Without this is possible to launch it as a JUnit test, (webserver must be running)
//otherwise it will fail with a Connection  error (webserver must be not running)
class Spring6jdbc3ApplicationTests {
    ...
    void testGetSpeaker() {
        ...
        Speaker speaker = restTemplate.getForObject("http://localhost:8080/speaker/{id}", Speaker.class, 326);
    ...
    @Test
    void testGetLastSpeaker() {
        RestTemplate restTemplate = new RestTemplate();

        Speaker speaker = restTemplate.getForObject("http://localhost:8080/speaker/last", Speaker.class);
    ...
    void testUpdateSpeaker() {
        ...
        Speaker speaker = restTemplate.getForObject("http://localhost:8080/speaker/{id}", Speaker.class, 326);
        speaker.setName(speaker.getName() + "Sr."); //Update the records
    ...
    void testDeleteSpeaker() {
        ...
        restTemplate.delete("http://localhost:8080/speaker/delete/{id}", 840);  
    }   
